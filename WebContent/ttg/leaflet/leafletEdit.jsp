									...
									<div class="form-group">
										...
										<div class="col-lg-8">
											<input type="text" id="skipUrl" class="form-control"
												name="dto.skipUrl" placeholder="链接地址" maxlength="512"
									...
									<div class="form-group">
										...
										<div class="col-lg-8">
											<input id="uploadLogo" type="file" class="file-loading"
												...
												id="oldImgname" name="dto.pic"
												value="<s:property value="dto.pic"/>" />
										</div>
									</div>
									<div class="form-group">
				...
				.fileinput(
						{
							...
							minImageHeight : 50,
							maxFileSize : 2 * 1024,
							initialPreview : [ "<img src='${fileHttpUrl}${dto.pic}' class='file-preview-image' alt='${dto.pic}' title='${dto.pic}'/>" ],
							enctype : 'multipart/form-data',
							uploadExtraData : function(previewId, index) {
								var oldImgname = $("#oldImgname").val();
								return {
									'dto.oldImgname' : oldImgname,
									'dto.id' : $("#id").val(),
		...
		$('#uploadLogo')
				.on(
						...
						function(event, data, previewId, index) {
							...
							if (response.error == "") {
								loadPage('leaflet_toEdit.action?dto.id=<s:property value="dto.id"/>');
				...
				.on(
						...
						function(event, data, previewId, index) {
							...
							if (response.error == "") {
								...
								var src = imgPrefix + imageName;

								var preview = $("#preview0").clone().html();
								preview = preview.replace(/#src/, src);
								preview = preview.replace(/#id/, id);
								preview = preview.replace(/#imageName/ig,
										imageName);
